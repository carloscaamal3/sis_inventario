...
async function imprimeFormatoOrdenCompra(osNumDoc, vm) {
  ...
  if (ordenCompraImpresion.length > 0 && ordenCompraDetImpresion.length > 0) {
    ...
    for (var i in ordenCompraDetImpresion) {
      ...
      doc.text(x_total - 2, valor_y, formato(eFormatos.MONEDA, cadena), { align: 'right' })
    ...
    doc.setFontSize(8)
    //GUARDA documento
    nomnreArchivo = 'ordencompra_' + osNumDoc + 'pdf'
    return doc.save(nomnreArchivo)
  }
}


export { imprimeFormatoOrdenCompra }
